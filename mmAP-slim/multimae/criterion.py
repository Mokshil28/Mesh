...
class MaskedL1Loss(nn.Module):
    """L1 loss with masking
    :param patch_size: Patch size (height, width)
    :param stride: Stride of task / modality
    :param norm_pix: Normalized pixel loss
    """

    def __init__(self, patch_size=(16, 16), stride=1, norm_pix=False):
        super().__init__()
        self.patch_size = patch_size
        self.stride = stride
        self.scale_factor_h = patch_size[0] // stride
        self.scale_factor_w = patch_size[1] // stride
        self.norm_pix = norm_pix

    def patchify(self, imgs, nh, nw):
        p1, p2 = self.scale_factor_h, self.scale_factor_w
        x = rearrange(imgs, "b c (nh p1) (nw p2) -> b (nh nw) (p1 p2 c)", nh=nh, nw=nw, p1=p1, p2=p2)
        return x

    def unpatchify(self, x, nh, nw):
        p1, p2 = self.scale_factor_h, self.scale_factor_w
        imgs = rearrange(x, "b (nh nw) (p1 p2 c) -> b c (nh p1) (nw p2)", nh=nh, nw=nw, p1=p1, p2=p2)
        return imgs

    def forward(self, input, target, mask=None):
        H, W = input.shape[-2:]
        nh, nw = H // self.scale_factor_h, W // self.scale_factor_w
