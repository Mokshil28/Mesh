...
DOMAIN_CONF = {    
    'angle': {
        ...
        'output_adapter': partial(SpatialOutputAdapter, num_channels=1, patch_size_full=(128,1), image_size = (128, 128)),
        'loss': MaskedL1Loss,
    },
    'doppler': {
        'channels': 1,
        ...
        'output_adapter': partial(SpatialOutputAdapter, num_channels=1, patch_size_full=(256,1), image_size = (256, 128)),
        'loss': MaskedL1Loss,
    },
    'range': {
        'channels': 1,
        ...
        'output_adapter': partial(SpatialOutputAdapter, num_channels=1, patch_size_full=(100,1), image_size = (100, 128)),
        'loss': MaskedL1Loss,
    },
}
...
def get_model(args):
    ...
    input_adapters = {
        domain: DOMAIN_CONF[domain]['input_adapter'](
            stride_level=DOMAIN_CONF[domain]['stride_level'],
            patch_size_full=DOMAIN_CONF[domain]['input_adapter'].keywords.get('patch_size_full'),
            image_size=DOMAIN_CONF[domain]['input_adapter'].keywords.get('image_size')
        )
        for domain in args.in_domains
    }
    # print(f"[DEBUG] Input adapters: {input_adapters}")
    ...
    output_adapters = {
        domain: DOMAIN_CONF[domain]['output_adapter'](
            stride_level=DOMAIN_CONF[domain]['stride_level'],
            patch_size_full=DOMAIN_CONF[domain]['output_adapter'].keywords.get('patch_size_full'),
            dim_tokens=args.decoder_dim,
            num_heads=args.decoder_num_heads,
            use_task_queries=args.decoder_use_task_queries,
            task=domain,
    ...
    # print(f"[DEBUG] Output adapters: {output_adapters}")
...
def train_one_epoch(
    ...
    for step, (x, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        
        # Normalize input data to [-1, 1]
        x = {
            key: 2 * (x[key] - x[key].min()) / (x[key].max() - x[key].min()) - 1
            for key in x
        }
        ...
        tasks_dict = {
            task: tensor.to(device, non_blocking=True).unsqueeze(1) if tensor.dim() == 3 else tensor.to(device, non_blocking=True)
