    ...
    for target_class in sorted(class_to_idx.keys()):
        ...
        for root, _, fnames in sorted(os.walk(target_dir, followlinks=True)):
            for fname in sorted(fnames):
                # External drives written by macOS may contain resource-fork
                # sidecars such as ._angle.npy.  They are not NumPy arrays.
...
class DatasetFolder(VisionDataset):
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]: 
        #TODO need a new __getitem__ for heatmap dataset
        """
        Args:
            index (int): Index
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        if index in self.cache:
            ...
            for task in self.tasks:
                ...
                sample = npy_loader(path) #pil_loader(path, convert_rgb=(task=='rgb'))
...
IMG_EXTENSIONS = ('.jpg', '.jpeg', '.png', '.ppm', '.bmp', '.pgm', '.tif', '.tiff', '.webp', '.jpx', '.npy')
...
class MultiTaskImageFolder(MultiTaskDatasetFolder):
    ...
    def __init__(
            ...
            transform: Optional[Callable] = None,
            target_transform: Optional[Callable] = None,
            loader: Callable[[str], Any] = npy_loader, # pil_loader,
            is_valid_file: Optional[Callable[[str], bool]] = None,
            prefixes: Optional[Dict[str,str]] = None,
            max_images: Optional[int] = None
    ):
