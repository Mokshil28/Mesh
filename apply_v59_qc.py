#!/usr/bin/env python3
"""Apply V59 manual QC: cut fall clips from user-provided timestamps."""
import json
import re
import subprocess
import sys
...
MANUAL_CLIPS = [
    ("0:50.3", "0:53", "v59 user manual"),
    ("1:00", "1:04", "v59 user manual"),
    ("1:12", "1:14", "v59 user manual"),
    ("1:24", "1:27", "v59 user manual"),
    ("1:28", "1:32", "v59 user manual"),
    ("5:09", "5:12", "v59 user manual - cut before camera cut"),
    ("7:32", "7:36", "v59 user manual - cut before camera cut"),
    ("7:51", "7:55", "v59 user manual"),
    ("8:45", "8:48", "v59 user manual"),
    ("9:06", "9:10", "v59 user manual"),
]

END_BEFORE_CAMERA = {6, 7}
YOUTUBE_URL = "http://youtube.com/watch?v=AIG_pn3kEyc"
...
def main():
    folder = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("data/fall_dataset_clips/059_V59")
    ...
    for i, (start_s, end_s, note) in enumerate(clips, 1):
        ...
        manifest.append({"clip": name, "mode": "manual_qc_v59", "start_frame": start_f, "end_frame": end_f, "duration_sec": round((end_f - start_f) / fps, 2), "note": note, "source_clip_v59": "user_manual"})

    (folder / "manual_timings.json").write_text(json.dumps(manual, indent=2) + "\n")
    (folder / "manifest.json").write_text(json.dumps(manifest, indent=2) + "\n")
    (folder / "video_info.json").write_text(json.dumps({"video_id": "V59", "folder": "059_V59", "youtube_url": YOUTUBE_URL, "note": f"{len(manual)} user manual clips"}, indent=2) + "\n")
    ...
    print(f"Cutting {len(manual)} V59 clips @ {fps:.3f} fps")
    ok = 0
    for entry in manual:
