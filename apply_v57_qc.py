#!/usr/bin/env python3
"""Apply V57 manual QC: cut fall clips from user-provided timestamps."""
import json
import re
import subprocess
import sys
...
MANUAL_CLIPS = [
    ("1:05", "1:08", "v57 user manual"),
    ("1:23", "1:26", "v57 user manual"),
    ("1:55", "1:59", "v57 user manual"),
    ("2:13", "2:17", "v57 user manual"),
    ("2:55", "2:59", "v57 user manual"),
    ("6:31", "6:35", "v57 user manual"),
    ("6:48", "6:51", "v57 user manual"),
    ("7:36", "7:39", "v57 user manual - start after camera cut"),
    ("8:14", "8:17", "v57 user manual"),
    ("8:30", "8:33", "v57 user manual"),
    ("13:46", "13:49", "v57 user manual"),
    ("13:58", "14:01", "v57 user manual"),
    ("16:28", "16:31", "v57 user manual"),
    ("16:38", "16:40", "v57 user manual - cut before camera cut"),
    ("16:46", "16:50", "v57 user manual"),
    ("18:12", "18:15.5", "v57 user manual"),
    ("18:25", "18:28", "v57 user manual"),
    ("18:38", "18:42", "v57 user manual - start after camera cut"),
    ("18:57", "18:59", "v57 user manual"),
    ("19:54", "19:57", "v57 user manual - start after camera cut"),
    ("20:00", "20:04", "v57 user manual"),
    ("20:25", "20:28.4", "v57 user manual"),
    ("20:32", "20:34.5", "v57 user manual"),
    ("20:57", "21:01", "v57 user manual"),
    ("21:25", "21:28", "v57 user manual"),
    ("22:00", "22:03", "v57 user manual"),
    ("22:14", "22:17", "v57 user manual"),
    ("22:33", "22:36", "v57 user manual"),
    ("22:44", "22:47", "v57 user manual"),
    ("23:16", "23:20", "v57 user manual"),
    ("23:28", "23:32", "v57 user manual"),
]

END_BEFORE_CAMERA = {14}
START_AFTER_CAMERA = {8, 18, 20}
YOUTUBE_URL = "https://www.youtube.com/watch?v=YllPDFb6oJI"
...
def apply_camera_qc(clips: list[tuple[str, str, str]], source: Path) -> list[tuple[str, str, str]]:
    ...
    for idx in END_BEFORE_CAMERA:
        ...
        if cuts:
            new_end = cuts[-1] - 0.2
            if new_end > t0 + 0.5:
                out[idx - 1] = (start_s, fmt_time(new_end), "v57 QC - end before camera cut")
    for idx in START_AFTER_CAMERA:
        ...
        if cuts:
            ...
            if new_start < t1 - 0.5:
                out[idx - 1] = (fmt_time(new_start), end_s, "v57 QC - start after camera cut")
    return out
...
def main():
    folder = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("data/fall_dataset_clips/057_V57")
    ...
    for i, (start_s, end_s, note) in enumerate(clips, 1):
        ...
        manifest.append({"clip": name, "mode": "manual_qc_v57", "start_frame": start_f, "end_frame": end_f, "duration_sec": round((end_f - start_f) / fps, 2), "note": note, "source_clip_v57": "user_manual"})

    (folder / "manual_timings.json").write_text(json.dumps(manual, indent=2) + "\n")
    (folder / "manifest.json").write_text(json.dumps(manifest, indent=2) + "\n")
    (folder / "video_info.json").write_text(json.dumps({"video_id": "V57", "folder": "057_V57", "youtube_url": YOUTUBE_URL, "note": f"{len(manual)} user manual clips"}, indent=2) + "\n")
    ...
    print(f"Cutting {len(manual)} V57 clips @ {fps:.3f} fps")
    ok = 0
    for entry in manual:
